...
if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
  ...
  // --- Audio Engine ---
  const shutterAudio = new Audio('https://cdnjs.cloudflare.com/ajax/libs/ion-sound/3.0.7/sounds/camera_flashing.mp3');
  shutterAudio.volume = 1.0;

  function playShutter() {
    shutterAudio.currentTime = 0;
    shutterAudio.play().catch(e => console.log('Audio playback failed', e));
  }
  ...
  if (cursor && cursorDot) {
    ...
    // Flash and Shutter effect on click
    window.addEventListener('mousedown', () => {
      playShutter();
      if (cameraFlash) {
        gsap.fromTo(cameraFlash, 
          { opacity: 0.8 }, 
  ...
  // Magnetic Heading
  const magneticHeading = document.querySelector('.magnetic-heading');
  const heroSection = document.getElementById('hero');
  
  if (magneticHeading && heroSection) {
    heroSection.addEventListener('mousemove', (e) => {
      const rect = heroSection.getBoundingClientRect();
      const heroCenterX = rect.left + rect.width / 2;
      const heroCenterY = rect.top + rect.height / 2;
      
      const x = e.clientX - heroCenterX;
      const y = e.clientY - heroCenterY;
      
      gsap.to(magneticHeading, {
        x: x * 0.02,
        y: y * 0.02,
        scale: 1.01,
        duration: 0.8,
